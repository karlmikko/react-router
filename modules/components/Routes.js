...
var Routes = React.createClass({
  ...
  statics: {
    ...
    handleCancelledTransition: function (transition) {
      ...
      if (reason instanceof Transition.Redirect) {
        replaceWith(reason.to, reason.params, reason.query);
      } else if (reason instanceof Transition.Abort) {
        goBack();
      }
    }
  ...
  componentWillMount: function () {
    ...
    if (ExecutionEnvironment.canUseDOM) {
      this.setStateFromPath(PathStore.getCurrentPath(), window.__ReactRouter_initialData || null);
